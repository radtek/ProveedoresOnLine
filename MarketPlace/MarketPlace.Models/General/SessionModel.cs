...
namespace MarketPlace.Models.General
{
    public static class SessionModel
    {
        ...
        public static List<int> CurrentThirdknowledgeOption()
        {
            List<int> oReturn = new List<int>();
            ...
            if (CurrentCompany != null && CurrentCompany.RelatedUser != null)
            {
                CurrentCompany.RelatedUser.All(x =>
                {
                    if (x.RelatedCompanyRole.RoleModule != null &&
                        x.RelatedCompanyRole.RoleModule.Count > 0)
                    {
                        List<SessionManager.Models.POLMarketPlace.Session_GenericItemModel> oModel = x.RelatedCompanyRole.RoleModule.Where(y => Convert.ToInt32(y.RoleModule) == (int)MarketPlace.Models.General.enumModule.ThirdKnowledgeInfo).Select(y => y.ModuleOption).FirstOrDefault();
